...
CACHE="$HERE/.cache"; IMG="$CACHE/corewaf-rig-base.qcow2"; mkdir -p "$CACHE"
REF="$(reg_var RIG_BASE_REF)"; SHA="$(reg_var RIG_BASE_SHA256)"
ok_sha() { [[ -s "$IMG" ]] && { [[ -z "$SHA" ]] || echo "$SHA  $IMG" | sha256sum -c --quiet - 2>/dev/null; }; }
if ok_sha; then echo "[base] $IMG present (sha ok)"; exit 0; fi
if [[ "${RIG_BASE_BUILD:-0}" == 1 || -z "$REF" ]]; then
    echo "[base] building locally with packer"; GOLDEN="$IMG" "$HERE/scripts/build-kit-base.sh" --force; exit 0
fi
...
mv -f "$f" "$IMG"; echo "$got  $(basename "$IMG")" > "$IMG.sha256"; echo "[base] fetched -> $IMG (sha256 $got)"
