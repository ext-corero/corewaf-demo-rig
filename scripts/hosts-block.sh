...
echo "# Browser (any OS, no hosts file needed): GUI http://gui-1.localhost:${RIG_HTTP_PORT:-8080}   Grafana http://grafana.localhost:${RIG_GRAFANA_PORT:-3000}"
...
if grep -qiE 'microsoft|wsl' /proc/version 2>/dev/null || [[ "${OS:-}" == Windows_NT ]]; then
  ...
  echo "# GUI http://gui-1.$RIG_DOMAIN:8080  (or http://localhost:${RIG_HTTP_PORT:-8080} with a Host header)"
fi
