#!/bin/sh
# ORCHESTRATOR TEMPLATE: rendered to config/stepca-init.sh (invoked via /bin/sh — exec bit not needed).
...
step ca init \
  ...
  --name="{{ .Env.STEPCA_NAME }}" \
  --dns="stepca,localhost,127.0.0.1,{{ .Env.FQDN }},stepca.{{ .Env.CS_DOMAIN }}" \
  --address=":9000" \
  --provisioner="admin" \
